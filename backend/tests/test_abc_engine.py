...
from app.models.models import AllocationLevel
...
class TestABCEngineDirectCosts:
    ...
    def test_single_cost_single_rule(self):
        """Un costo su un centro di costo con una regola di allocazione."""
        engine = ABCEngine()
        period_id = uuid4()
        ...
        allocation_rules = [
            AllocationRuleRecord(
                rule_id=uuid4(),
                level=AllocationLevel.COST_TO_ACTIVITY.value,
                source_cost_center_id=cost_records[0].cost_center_id,
        ...
        activities = [allocation_rules[0].target_activity_id]
        services = [uuid4()]
    ...
    def test_multiple_costs_split_by_percentage(self):
        """Due costi su un CC con allocazione percentuale a due attività."""
        engine = ABCEngine()
        period_id = uuid4()
        cc_id = uuid4()
        act_a = uuid4()
        ...
        cost_records = [
            CostRecord(cost_item_id=uuid4(), cost_center_id=cc_id, cost_type="diretto", amount=Decimal("1000")),
            CostRecord(cost_item_id=uuid4(), cost_center_id=cc_id, cost_type="diretto", amount=Decimal("500")),
        ]
        ...
        result = engine.calculate(
            ...
            allocation_rules=allocation_rules,
            service_revenues=[],
            activity_ids=[act_a, act_b],
            service_ids=[uuid4()],
            support_activity_ids=[],
        )

        # 1500 * 0.6 = 900, 1500 * 0.4 = 600
    ...
    def test_no_allocation_rules_warning(self):
        """Se non ci sono regole per un CC, il costo non viene allocato."""
        engine = ABCEngine()
        period_id = uuid4()
        cc_id = uuid4()
        ...
        assert len(result.warnings) > 0
        assert any("Nessuna regola di allocazione" in w for w in result.warnings)

    def test_labor_cost_accumulation(self):
        """Testa l'accumulo di costi del personale su un'attività."""
        engine = ABCEngine()
        period_id = uuid4()
        act_id = uuid4()
        emp_id = uuid4()
        ...
        labor_records = [
            LaborRecord(
                employee_id=emp_id, activity_id=act_id,
...
class TestABCEngineSupportAllocation:
    ...
    def test_simple_support_allocation(self):
        """Un'attività di supporto distribuisce il suo costo a un'attività primaria."""
        engine = ABCEngine(max_iterations=10, convergence_threshold=0.001)
        period_id = uuid4()
        support_act = uuid4()
        primary_act = uuid4()

        # Il support ha costo diretto di 1000
        cost_records = [
            CostRecord(cost_item_id=uuid4(), cost_center_id=uuid4(), cost_type="diretto", amount=Decimal("1000")),
        ]

        labor_records = [
            LaborRecord(
                employee_id=uuid4(), activity_id=support_act,
                hours=Decimal("10"), hourly_cost=Decimal("20"),
                allocation_pct=Decimal("1"),
            ),
        ]

        # Regola: support -> primary 100%
        allocation_rules = [
            AllocationRuleRecord(
        ...
        # Il costo del support (1000 + 200 = 1200) viene ribaltato su primary
        support_total = Decimal("1200.00")
        assert result.activity_costs[primary_act].inbound_allocated == support_total

    def test_no_support_activities(self):
        """Se non ci sono attività di supporto, il ribaltamento non fa nulla."""
        engine = ABCEngine()
        period_id = uuid4()
...
class TestABCEngineServiceAllocation:
    ...
    def test_activity_to_service(self):
        """Un'attività distribuisce il suo costo ai servizi."""
        engine = ABCEngine()
        period_id = uuid4()
        act_id = uuid4()
        svc_a = uuid4()
        ...
        # Attività ha 1000 di costo
        labor_records = [
            LaborRecord(
                employee_id=uuid4(), activity_id=act_id,
        ...
        allocation_rules = [
            AllocationRuleRecord(
                rule_id=uuid4(), level="attivita_a_servizio",
...
class TestABCEngineRevenues:
    ...
    def test_margin_calculation(self):
        """Calcolo corretto dei margini per un servizio."""
        engine = ABCEngine()
        period_id = uuid4()
        svc_id = uuid4()
        act_id = uuid4()
        ...
        service_revenues = [
            ServiceRevenueRecord(service_id=svc_id, revenue=Decimal("2000"), output_volume=Decimal("100")),
        ]
        ...
        assert svc.total_cost == Decimal("1000")
        assert svc.gross_margin == Decimal("1000")
        assert svc.margin_pct == Decimal("50.00")
        assert svc.cost_per_unit == Decimal("10.0000")
        assert svc.output_volume == Decimal("100")

    def test_zero_revenue_no_crash(self):
        """Se il ricavo è zero, margin_pct deve essere None (divisione per zero)."""
        engine = ABCEngine()
        period_id = uuid4()
        svc_id = uuid4()
        act_id = uuid4()
        ...
        service_revenues = [
            ServiceRevenueRecord(service_id=svc_id, revenue=Decimal("0"), output_volume=Decimal("50")),
        ]
        ...
        svc = result.service_results[svc_id]
        assert svc.margin_pct is None
        assert svc.cost_per_unit == Decimal("0.0000")


class TestABCEngineEdgeCases:
    """Testa casi limite ed edge cases."""

    def test_empty_inputs(self):
        """Senza costi, attività o servizi, il risultato deve essere vuoto."""
        engine = ABCEngine()
        result = engine.calculate(
            period_id=uuid4(),
            cost_records=[],
    ...
    def test_partial_allocation_normalized(self):
        """Se le percentuali non sommano a 1, devono essere normalizzate."""
        engine = ABCEngine()
        period_id = uuid4()
        cc_id = uuid4()
        act_a = uuid4()
        ...
        # Somma solo 0.5 + 0.3 = 0.8 (deve essere normalizzato)
        allocation_rules = [
            AllocationRuleRecord(
                rule_id=uuid4(), level="costo_ad_attivita",
                source_cost_center_id=cc_id, target_activity_id=act_a,
        ...
        # Normalizzato: 0.5/0.8=0.625 e 0.3/0.8=0.375
    ...
    def test_unallocated_cost_warning(self):
        """Se ci sono costi non allocati ai servizi, deve comparire un warning."""
        engine = ABCEngine()
        period_id = uuid4()
        cc_id = uuid4()
        act_id = uuid4()
        ...
        allocation_rules = [
            ...
            # Nessuna regola attività->servizio: il costo resta non allocato
        ]
        ...
        result = engine.calculate(
            ...
            allocation_rules=allocation_rules,
            service_revenues=[
                ServiceRevenueRecord(service_id=svc_id, revenue=Decimal("5000"), output_volume=Decimal("50")),
                ServiceRevenueRecord(service_id=svc_id2, revenue=Decimal("5000"), output_volume=Decimal("50")),
            ],
            activity_ids=[act_id],
            service_ids=[svc_id, svc_id2],
            support_activity_ids=[],
        )
